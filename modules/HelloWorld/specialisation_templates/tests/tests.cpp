...
namespace Test {
TEST(TemplateTests, step2_templates) {
  {
...
auto test_max(auto a, auto b) {
  ...
  TEST(TemplateTests, step2_check_templates) {
    {
  ...
  const char * test_special_max(const char * a, const char * b) {
    return (std::string{a} > std::string{b}) ? a : b;
  }
   TEST(TemplateTests, step2_check_specialisation_templates) {
    {
      bool result = std::is_same_v<decltype(local::my_max("","")), decltype(test_special_max("",""))  >;
